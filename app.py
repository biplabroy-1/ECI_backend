...
def analyze_pdf():
    try:
        ...
        image_paths, output_folder = convert_pdf_to_images(upload_path, start_page=3)

        if not image_paths:
            return jsonify({"error": "Failed to convert PDF to images"}), 500

        # Step 3: Run Groq OCR
        processed_result = process_all_images(image_paths, output_folder, os.getenv("GROQ_API_KEY"))
